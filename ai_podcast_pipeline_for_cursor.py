...
OPUS47_SCRIPT_TUNING_APPENDIX = (
    ...
    "- Target spoken length: roughly 5.5 to 7.5 minutes for the main episode body, before the separate intro and outro are added.\n"
    "- Target script length: 4,200 to 6,100 characters. Treat 6,700 characters as a hard ceiling unless extra context is required for factual accuracy.\n"
    "- Make this roughly 10% longer than the current shorter draft target: add useful context, stakes, and color where it improves the story, but avoid returning to long, repetitive background.\n"
    "- The research brief may include 5 candidate stories. Select the strongest 3 to 4 stories for the final script; use 4 only when the fourth is genuinely strong and fits naturally within the target length.\n"
    "- Prefer the biggest, most broadly relevant AI stories, especially frontier lab and major AI company model, tool, product, agent, platform, or developer releases from OpenAI, Anthropic, Google or DeepMind/Gemini, xAI, Amazon/AWS, Meta, Microsoft, NVIDIA, and similarly important AI companies.\n"
    "- Do not choose obscure startup announcements, narrow enterprise integrations, routine funding items, or minor feature releases unless they are clearly more consequential than the major-company alternatives because they show broad adoption, a real technical breakthrough, or an important shift in how people use AI.\n"
    "- If script-writing web search is enabled, use it only when needed to verify a key detail, fill an important gap, find a stronger quote/reaction, or check for a material update. It is not required to write the script, and you should not do broad new research when the supplied material is enough.\n"
    "- For each story, lead with the concrete news, then focus on impact: why it matters, who it affects, what changed, what users or developers can do differently, and what tension, tradeoff, or strategic move it reveals.\n"
    "- Do not read off metrics, figures, percentages, benchmark scores, token counts, pricing minutiae, or highly technical implementation details just because they are available. Use them only when they support a clear point, comparison, tradeoff, user impact, or concrete stakes.\n"
    "- Prioritize interesting quotes from articles, company representatives, users, developers, and social media when they reveal why the story matters. Prefer one strong quote or reaction per story, then briefly explain what it shows.\n"
    "- Spend roughly 2 to 3 concise spoken paragraphs on each major story when the source material supports it.\n"
...
"Today we will be..."
Since we will add intro and outro on our end, please do not make references to the podcast, simply generate the script going over the topics provided. The script should be written in plain, conversational language that is casual, informed, and assumes the listener is familiar with AI models generally and just wants to hear the facts. Make the episode more interesting by using the strongest available quotes, credible social media reactions, user/developer responses, product specifics, contradictions, and stakes from the source material. Do not rely on generic AI commentary; for every story, explain what is surprising, consequential, controversial, useful, or revealing. When the provided material is lengthy or technical, distill the most newsworthy and engaging storylines while preserving key facts and the best quotes, not dense technical detail. If the research brief presents 5 candidate stories, select the strongest 3 to 4 for the final script; use 4 only when the fourth is genuinely strong and fits naturally within the target length. Prefer the biggest, most broadly relevant stories, especially frontier AI lab and major AI company releases from OpenAI, Anthropic, Google or DeepMind/Gemini, xAI, Amazon/AWS, Meta, Microsoft, NVIDIA, and similarly important AI platforms. Do not choose obscure startup announcements unless they are clearly more consequential than the major-company alternatives because they show broad adoption, a real technical breakthrough, or an important shift in how people use AI. If web search is enabled for this script-writing step, use it only when needed to verify a key detail, fill an important gap, find a stronger quote or reaction, or check for a material update. Web search is optional for script writing and broad new research is unnecessary when the supplied material is enough. Prioritize clarity for listening, but keep the pacing lively and concise. Design the content for a natural 5.5-7.5 minute main-body runtime with smooth transitions and no explicit headings or titles. Podcast script should contain between 4,200 and 6,100 characters, with 6,700 characters as a hard ceiling unless extra context is required for factual accuracy. Make this roughly 10% longer than the current shorter draft target by adding useful context, stakes, quotes, reactions, and color where it improves the story, while avoiding long, repetitive background or broad framing. Only read off metrics, figures, percentages, benchmark scores, token counts, pricing details, or highly technical information when they support a clear point, comparison, tradeoff, user impact, or concrete stakes. Prioritize interesting quotes from articles, company representatives, users, developers, and social media when they reveal why the story matters, then briefly explain what the quote or reaction shows.
...
Topic to generate Title and Description based:'''],
        ...
        [8, "Do not select topics previously covered", "Below are previously covered items from recent episodes. Treat this as a strong freshness filter: prefer not to cover the same company announcement, product release, benchmark, or story angle again. Only include a previously covered topic when there is a genuinely major update or significant new feedback worth covering, such as specific post-release user/developer reactions, fresh benchmarks or evals, meaningful adoption or use-case evidence, availability or pricing changes, newly reported limitations, credible controversy, safety issues, or other materially new evidence. If a follow-up is included, focus only on what is new since the previous episode; do not recap or re-explain the original story except for one brief sentence of context. Explicitly note it is a follow-up and explain why the update clears the bar. If the new angle is thin, repetitive, or only a minor mention, choose a different current story instead. Previously Covered News Items below:"],
        [9, "Custom Topic", ""],
        [10, "Daily AI News Search", """Role: You are an AI news research editor for AI Convo Cast.

Goal: Produce a source-grounded research brief with the 5 strongest recent AI story candidates from the last 24-48 hours, suitable for a podcast script. Expand to 72 hours only if there are not enough strong, current stories.

Success criteria:
- Select 5 strong candidate stories. Prioritize the biggest AI news with broad listener relevance: frontier model releases, major tool/product launches, agent or workflow updates, platform changes, notable research, developer-facing changes, and major announcements from OpenAI, Anthropic, Google/DeepMind/Gemini, xAI, Amazon/AWS, Meta, Microsoft, NVIDIA, and similarly important AI companies.
- Deprioritize obscure startup announcements, small funding items, routine integrations, narrow enterprise announcements, and incremental feature releases unless they clearly signal a broader industry shift, major adoption, a technical breakthrough, or an important new capability users will care about.
- The script writer will choose the final 3 to 4 stories from these 5 candidates, so include enough variety and quality for that selection.
...
- Use the appended prior-coverage list as a strong freshness filter. Prefer not to cover the same story, company announcement, product launch, benchmark, or angle again. Only include a previously covered topic when there is a genuinely major update or significant new feedback worth covering, such as current, sourced post-release user/developer feedback, fresh benchmarks or evals, meaningful adoption or use-case evidence, availability or pricing changes, newly reported limitations, credible controversy, safety issues, or other materially new evidence. If covering a follow-up, report only the new development and give at most one brief context sentence about the original story.
...
OPENING LINE
Today we will be... <very brief list of items, in one sentence, no bullets>.

OBJECTIVE
Deliver a clear, conversational walkthrough of the provided AI news items. Focus on the impact of each story and why it matters, with smooth transitions and no summaries or conclusions.

REASONING & STYLE
- Think privately to plan structure, pick the clearest order, and craft natural transitions. Do NOT reveal your reasoning or process.
- Use light creative phrasing for rhythm and clarity, without adding unverified facts.

STRICT DO-NOTS
- No mid-episode recaps, no thematic tie-ups, no closing summary.
- No generic industry takes, no "Imagine a world...", no "Reflecting on these stories...".
- No stock-price play-by-play or market speculation.
- No headings, bullets, numbering, markdown, hyperlinks, citations, or any word/phrase ending in ".com".
- No "speaking of" or corny transitions. Just move naturally to the next topic.

CONTENT RULES
- Use only facts contained in the story details below or verified through optional script-stage web search; do not invent.
- When available in the story details or optional search, include short quotes from named company representatives, article sources, users, developers, or social media reactions when they reveal why the story matters. Keep direct quotes under 25 words.
- Assume the listener already knows common AI concepts; keep explanations tight and plain.
- The research brief may include 5 candidate stories. Select the strongest 3 to 4 stories for the final script; use 4 only when the fourth is genuinely strong and fits naturally within the target length.
- When choosing from the 5 candidate stories, prefer the biggest, most broadly relevant stories, especially frontier AI lab and major AI company releases from OpenAI, Anthropic, Google or DeepMind/Gemini, xAI, Amazon/AWS, Meta, Microsoft, NVIDIA, and similarly important AI platforms.
- Do not choose obscure startup announcements unless they are clearly more consequential than the major-company alternatives because they show broad adoption, a real technical breakthrough, or an important shift in how people use AI.
- If script-writing web search is enabled, use it only when needed to verify a key detail, fill an important gap, find a stronger quote/reaction, or check for a material update. It is not required to write the script, and broad new research is unnecessary when the supplied material is enough.
- Aim for a concise 5.5-7.5 minute main-body listen, roughly 10% longer than the current shorter draft target; total length 4,000-5,500 characters, with 6,000 as a hard ceiling. Add useful context, stakes, and color where it improves the story, while avoiding long, repetitive background or broad framing.
- Focus each story on impact: why it matters, who it affects, what changed, what users or developers can do differently, and what tension, tradeoff, or strategic move it reveals.
- Do not read off metrics, figures, percentages, benchmark scores, token counts, pricing details, or highly technical information just because they are available. Use them only when they support a clear point, comparison, tradeoff, user impact, or concrete stakes.
- Prefer one strong quote or reaction per story, then briefly explain what it shows.

TEXT NORMALIZATION
- Remove dashes from model/product names (e.g. GPT-4.5 -> GPT 4.5).
- When converting to speech: replace "GPT 4o" with "GPT Four-Oh" and "DALL-E" with "Dolly".

OUTPUT FORMAT
- Pure plain text (UTF-8). No headings, bullets, or numbered lists.
- Begin with the required opening line above, then flow directly into the first story.
- Use smooth sentence-level transitions between items; do not add section labels or recaps.

MATERIAL TO USE (facts come from below or verified optional script-stage search)
...
def call_openai_model(prompt, model="gpt-4o", temperature=0.8, web_search=False):
    ...
    # Keep a safety guard, but do not make the script-length target a hard cutoff.
        ...
        for i, step in enumerate(steps):
            ...
            try:
                ...
                if model_override is None and model_id_override is None:
                    ...
                    uses_openai_web_search = (
                        web_search_enabled
                        and not str(model_to_use).lower().startswith(('claude-', 'gemini-'))
                    )
                    # Fallback for OpenAI client if Responses API web search is not available.
                    if uses_openai_web_search and not hasattr(client, 'responses'):
                        msg = "❌ Web search requested but your OpenAI Python package does not support responses.create. Please upgrade openai to the latest version."
                        log_error(msg)
