...
OPUS47_SCRIPT_TUNING_APPENDIX = (
    ...
    "- Target spoken length: roughly 5 to 7 minutes for the main episode body, before the separate intro and outro are added.\n"
    "- Target script length: 3,800 to 5,500 characters. Treat 6,000 characters as a hard ceiling unless extra context is required for factual accuracy.\n"
    "- Make this roughly 20% longer than the shorter draft target: add useful context, stakes, and color where it improves the story, but avoid returning to long, repetitive background.\n"
    "- Cover 3 distinct stories when the source material supports it. Use 2 only when there are not 3 strong current stories or when 3 would force shallow coverage.\n"
