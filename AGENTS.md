...
- `.github/workflows/ai_podcast_pipeline.yml` is the main pipeline: targets Sun–Thu 4pm Eastern via Google Cloud Scheduler, plus a native GitHub fallback and manual `workflow_dispatch`. Normal manual runs leave `scheduled_run` and `dry_run` unchecked. Automatic runs skip after a manual trigger or prior automatic generation that day. See `PODCAST_SCHEDULING.md` for setup, free-tier limits, validation, and token renewal. It runs `ai_podcast_pipeline_for_cursor.py` in the `production` environment.
