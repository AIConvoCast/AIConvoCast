...
- `.github/workflows/ai_podcast_pipeline.yml` is the main pipeline: scheduled Sun–Thu 3pm Eastern, plus manual `workflow_dispatch` (with `custom_topic`, `workflow_id`, `force_run` inputs). It runs `ai_podcast_pipeline_for_cursor.py` in the `production` environment.
